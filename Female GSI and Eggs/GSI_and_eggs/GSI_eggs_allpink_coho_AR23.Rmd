...
---
12/04/23
Ok at some point we're making a combined rmd for everything where everything works, and it spits out clean graphs and results.
- and I mean for everything, not just this GSI/egg paper
- 1st half of this doc is summarized and nice, and then we do some graph/experimenting that is...uh... less organized
-anyway, I'm going to the results summary block to get the ##'s I need for the full coho dataset (which is what we have here, right?)
...
PINK Y2 GSI
12/04/23- looks like I tested date significance here
```{r}
...
COHO year 1: GSI 
- this is the full coho dataset, I believe , 12/04/23
```{r}
...
#12/04/23
#ok so we've already tested that length isn't in play here? or we did that before? *sigh*
names(c.GSI.clean)

mod_test_c <- lm(GSI ~ Length..mm. + Wild.or.Hatch, c.GSI.clean)
summary(mod_test_c)
mod_test_c_2 <- lm(GSI ~ Wild.or.Hatch, c.GSI.clean)
summary(mod_test_c_2)

AIC(mod_test_c, mod_test_c_2)
...
RESULTS SUMMARY
- I beleive this is the most useful part - AR 12/04/23
```{r}
#GSI:
p1.GSI.t.test
p2.GSI.t.test
...
setwd("/Users/alexandrareich/Desktop/THESIS TIME!/Compilation of GITHUB project code/Relaxed_selection_proj_reproducible_science/Figs")
